...
@property (weak, nonatomic) UIButton *flashButton;//闪光灯按钮
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    //拍照按钮
    UIButton *takeButton =[[UIButton alloc]initWithFrame:(CGRect){self.view.frame.size.width/2-(takeButtonHeight-20)/2,self.view.frame.size.height-takeButtonHeight+5,takeButtonHeight-20,takeButtonHeight-20}];
    [takeButton setImage:[UIImage imageNamed:@"camera"] forState:UIControlStateNormal];
    ...
    //闪光灯按钮
    UIButton * flashButton =[[UIButton alloc]initWithFrame:(CGRect){kWindowWidth- takeButtonHeight/2-5,0,takeButtonHeight/2,takeButtonHeight/2}];
    [flashButton setImage:[UIImage imageNamed:@"icon_btn_camera_flash_auto"] forState:UIControlStateNormal];
    ...
    //自动闪光灯按钮
    UIButton * flashAutoButton =[[UIButton alloc]initWithFrame:(CGRect){kWindowWidth- takeButtonHeight/2*4,0,takeButtonHeight/2,takeButtonHeight/2}];
    ...
    flashAutoButton.tag=1;
    flashAutoButton.alpha=0;
    ...
    flashOffButton.tag=3;
    flashOffButton.alpha=0;
    ...
    //切换摄像头按钮
    UIButton * toggleButton =[[UIButton alloc]initWithFrame:(CGRect){{self.view.frame.size.width-takeButtonHeight/2-takeButtonHeight/4+10 ,kWindowHeight-flashAutoButton.frame.size.height-20},flashAutoButton.frame.size}];
    ...
    //退出相机
    UIButton *cancelButton =[[UIButton alloc]initWithFrame:(CGRect){10,10,40,30}];
    ...
    _preView = preView;
    //预览相机图片
    UIImageView *preImageView =[[UIImageView alloc]initWithFrame:(CGRect){0,0,self.view.frame.size.width,self.view.frame.size.height}];
    ...
    //取相册
    UIButton *albumBtn =[[UIButton alloc]initWithFrame:(CGRect){10,self.view.frame.size.height-70,50,50}];
    albumBtn.backgroundColor=[UIColor whiteColor];
    [albumBtn addTarget:self action:@selector(cancelButtonClick) forControlEvents:UIControlEventTouchUpInside];
    _albumBtn = albumBtn;
    ...
    if ([captureSession canSetSessionPreset:AVCaptureSessionPresetHigh]) {//设置分辨率
...
-(void)flashClick:(UIButton *)btn
{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        _flashAutoButton.alpha=alphaF;
        _flashOnButton.alpha=alphaF;
        _flashOffButton.alpha=alphaF;
    }];
 ...
 *  闪光灯自动点击
 */
-(void)flashAutoClick:(UIButton *)btn
{
    [self setFlashMode:AVCaptureFlashModeAuto];
    [self setFlashModeButtonStatus];
    [_flashButton setImage:btn.imageView.image forState:UIControlStateNormal];
    [self flashClick:_flashButton];
}
/**
 *  闪光灯开启点击
 ...
 *  关闭闪光灯
 */
-(void)flashOffClick:(UIButton *)btn
{
...
{
    //    _preImageView.image
    //    UIImageWriteToSavedPhotosAlbum(_preImageView.image, nil, nil, nil);

    [[UIApplication sharedApplication] setStatusBarHidden:NO withAnimation:NO];
    [self dismissViewControllerAnimated:YES completion:^{
        
        [self.delegate cameraImage:_preImageView.image];
    }];
...
-(void)tapScreen:(UITapGestureRecognizer *)tapGesture{
    [UIView animateWithDuration:0.5 animations:^{
        _flashAutoButton.alpha=0;
        _flashOnButton.alpha=0;
        _flashOffButton.alpha=0;
    }];
