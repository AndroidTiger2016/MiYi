...
@interface MiYiMyLoveVC ()<UITableViewDataSource,UITableViewDelegate,MiYiBaseCommunityDelegate>
...
@implementation MiYiMyLoveVC

- (void)viewDidLoad {
    [super viewDidLoad];
    _page=0;
    
    UITableView *tableView =[[UITableView alloc]initWithFrame:(CGRect){0,0,kWindowWidth,kWindowHeight-40-64}];
...
-(void)footerRefresh
{
    ...
    WS(ws);
    [MiYiPostsRequest favoriteTopicsTopicUserUid:[MiYiUserSession shared].session.uid topic_page:[NSString stringWithFormat:@"%ld",(long)_page+1] json:^(id json) {
        NSLog(@"%@",json);
...
{
    MiYiBaseCommunityCell *cell =[MiYiBaseCommunityCell cellWithTableView:tableView identifier:@"MiYiMyLoveVC"];
    cell.delegate=self;
